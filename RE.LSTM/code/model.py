...
from keras.layers import Input, Embedding, Dropout, Dense, Lambda
from keras.layers import LSTM, Bidirectional
from keras.layers import concatenate, Permute, Reshape
...
#######################################  function  ########################################
### -------------  LSTM baseline  ------------- ###
def LSTMbaseline(embedding_matrix, max_sent_len, n_out, n_rnn=1, bidirection=False):
    # Take sentence encoded as indices and convert it to embeddings
    sent_input = Input(shape=(max_sent_len,), dtype='int32', name='sent_input')
    word_embeddings = Embedding(input_dim=embedding_matrix.shape[0], output_dim=embedding_matrix.shape[1], input_length=max_sent_len, weights=[embedding_matrix], mask_zero=True, trainable=False) (sent_input)
    ...
    entity_markers = Input(shape=(max_sent_len,), dtype='int8', name='entity_markers')
    pos_embeddings = Embedding(input_dim=5, output_dim=3, input_length=max_sent_len, mask_zero=True, embeddings_regularizer=l2(), trainable=True) (entity_markers)

    # Merge word and position embeddings and apply the specified amount of RNN layers
    x = concatenate([word_embeddings, pos_embeddings])
    ...
    output = Dense(n_out, activation="softmax", name='output') (sentence_vector)

    model = Model(inputs=[sent_input, entity_markers], outputs=[output])
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

    return model

### -------------  CNN  ------------- ###
def CNN(p, embedding_matrix, max_sent_len, n_out):
    ...
    # Take token markers that identify entity positions, convert to position embeddings
    entity_markers = Input(shape=(2, max_sent_len,), dtype='int8', name='entity_markers')
    pos_embeddings = Embedding(input_dim=(max_sent_len*2)+1, output_dim=3, input_length=max_sent_len, mask_zero=False, embeddings_regularizer=l2(), trainable=True)
    pos_embeddings = TimeDistributed(pos_embeddings, name='pos_embedding') (entity_markers)

    pos_embeddings = Permute((2,1,3)) (pos_embeddings)
    pos_embeddings = Reshape((max_sent_len, 3*2)) (pos_embeddings)

    # Merge word and position embeddings and apply the specified amount of CNN layers
    x = concatenate([word_embeddings, pos_embeddings])

    x = MaskedConvolution1D(nb_filter=256, filter_length=3, border_mode='same') (x)
    sentence_vector = MaskedGlobalMaxPooling1D() (x)

    sentence_vector = Lambda(lambda l: K.tanh(l)) (sentence_vector)

    # Apply softmax
    sentence_vector = Dropout(p['dropout1'])(sentence_vector)
    main_output = Dense(n_out, activation="softmax", name='main_output')(sentence_vector)

    model = Model(inputs=[sent_input, entity_markers], outputs=[main_output])
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
